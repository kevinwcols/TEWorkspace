...
public class App {
    ...
    private List<CatalogItem> userItems = new ArrayList<>();


    final int ITEM_TYPE_FIELD = 0;
    final int NAME_FIELD = 1;
    final int CREATOR_FIELD = 2;
    final int DATE_FIELD = 3;
    ...
    private void initialize() throws FileStorageException {
        ...
        List<String> memberFileContents = FileStorageService.readContentsOfFile(FILE_BASE_PATH+"members.dat");

          for (int i = 0; i < memberFileContents.size(); i++) {
              String[] memberDataRow = memberFileContents.get(i).split(FIELD_DELIMITER);
              if (memberDataRow.length == 3) {
                  Member member = new Member(memberDataRow[0], memberDataRow[1]);
                  memberName = member.toString();
                  memberFileName = memberDataRow[2];
              }

              List<String> fileContents = FileStorageService.readContentsOfFile(FILE_BASE_PATH + memberFileName);
              for (int j = 0; j < fileContents.size(); j++) {
                  if (!fileContents.isEmpty()) {
                      String[] dataRow = fileContents.get(j).split(FIELD_DELIMITER);
                      if (dataRow.length == 4) {
                          if (dataRow[0].equals("book")) {
                              Book book = new Book(dataRow[1], dataRow[2], LocalDate.parse(dataRow[3]));
                              book.registerItem();
                              userItems.add(book);
                          } else if (dataRow[0].equals("movie")) {
                              Movie movie = new Movie(dataRow[1], dataRow[2], LocalDate.parse(dataRow[3]));
                              movie.registerItem();
                              userItems.add(movie);
                          } else if (dataRow[0].equals("tool")) {
                              Tool tool = new Tool(dataRow[1], dataRow[2], Integer.parseInt(dataRow[3]));
                              tool.registerItem();
                              userItems.add(tool);
                          }
                      }
                  }
                  // add member names and catalog items
                  catalog.put(memberName, userItems);
              }
          }
