...
public class Exercise01_StoreOrders {
    ...
    public int[] createOrder() {
        int[] orderSally = { 10, 40, 31, 41 };  //initializing array
        return orderSally;
    }
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        ...
        int largeCheesPizza = 14;
        int revenueSally = 0;
        for (int i = 0; i < orders.length; i++) {
            if ( orders[i] == 10) {
                revenueSally = revenueSally + smallCheesePizza;
            }
            else if ( orders[i] == 20) {
                revenueSally = revenueSally + mediumCheesePizza;
            }
            else  if ( orders[i] == 30) {
                revenueSally = revenueSally + largeCheesPizza;
            }
        }
        return revenueSally;
    }
