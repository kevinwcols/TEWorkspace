...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		if (animalName == null)
			groupName = "unknown";
		else if(animalName.toLowerCase().equals("rhino"))
			groupName = "Crash";
		else if(animalName.toLowerCase().equals("giraffe"))
			groupName = "Tower";
		else if(animalName.toLowerCase().equals("elephant"))
			groupName = "Herd";
		else if(animalName.toLowerCase().equals("lion"))
			groupName = "Pride";
		else if(animalName.toLowerCase().equals("crow"))
			groupName = "Murder";
		else if(animalName.toLowerCase().equals("pegion"))
			groupName = "Kit";
		else if(animalName.toLowerCase().equals("flamingo"))
			groupName = "Pat";
		else if(animalName.toLowerCase().equals("deer"))
			groupName = "Herd";
		else if(animalName.toLowerCase().equals("dog"))
			groupName = "Pack";
		else if(animalName.toLowerCase().equals("crocodile"))
			groupName = "Float";
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		int arrayLength = words.length; //length of the array of strings passed.

		Map<String,Integer> map = new HashMap<>(); //Hashmap to store the count

		for(int i = 0 ; i < arrayLength ; i++) { //Traversing each string using for-loop.
			String str = words[i]; //Each String

			int lenString = str.length(); //length of the string

			String last = str.substring(lenString-2,lenString); //substring of last two chars

			int count = 0; //count for this substring.

			for(int j = 0; j < lenString - 2; j++) {//traversing the string characters to check the count.
				if(str.substring(j,j+2).equals(last)) {
					count++;//Increase count by 1 if the substring matches the substring of last 2 chars.
				}
			}
			map.put(str, count); //store the count in the map.
		}
