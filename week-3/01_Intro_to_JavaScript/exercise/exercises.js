...
function oddOnly(arr) {
	return arr.filter(n => n % 2 !== 0); // if a number is even, remove it
}

// more info on array filter function: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter
...
/* 	var firstTwoChars = word.substring(0,2);
	var lastTwoChars = word.substring(word.length-2,word.length);  */
	...
	if (word.substring(0,2) === word.substring(word.length-2,word.length)) {
	 ...
	 /* if (word.length == 2 || word.substring(0,2) == word.substring(word.length-2,word.length)) {
        return true
      } 
	  else if ((word.substring(0,2) !== 'ed' && word.substring(word.length-2,word.length) !== 'ed')) {
		return true
	  }
	  else {
        return false
...
function cigarParty (num, isWeekend) {
	if ( (isWeekend && num >= 40) || (!isWeekend && num >= 40 && num <= 60) ) {
		return true
	} return false
}
...
/* function cigarParty (num, isWeekend) {
	if (isWeekend && num >= 40)  {
		return true
	} else if (!isWeekend && num >= 40 && num <= 60) {
		return true
	}  return false
}  */
...
function filterEvens(arr) {
	return arr.filter(n => n % 2 === 0); // if a number is odd, remove it  (opposite of #5)
}
