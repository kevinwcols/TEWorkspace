...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		int lstStr = stringList.size();
		String[] tempStr2 = new String[lstStr];
		for(int i=0;i<lstStr;i++) {
			tempStr2[i] = stringList.get(i);
		}
		return tempStr2;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		List<Double> newDoubleArr = new ArrayList<Double>();
		double tempDouble;
		if(intArray.length > 0) {
			for(int i = 0; i < intArray.length; i++) {
				tempDouble = intArray[i] / 2.0;
				newDoubleArr.add(tempDouble);
			}
		}
		return newDoubleArr;
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		List<Integer> intOddValues = new ArrayList<Integer>();

		for(int i = 0; i < integerArray.length; i++) {
			if(integerArray[i] % 2 == 1) {
				intOddValues.add(integerArray[i]);
			}
		}
		return intOddValues;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (int i = 0; i < integerList.size(); i++) {
			if (integerList.get(i) == intToFind) {
				...
				else if ( valFoundTwice != true ) {
					valFoundTwice = false;
				}
			}
		}
		return valFoundTwice;
	}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for(int i = 0; i < integerArray.length; i++) {
			if((integerArray[i] % 3 == 0) && (integerArray[i] % 5 == 0)) {
				...
				newFizzBuzzList.add(integerArray[i].toString());
			}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> newInterwovenList = new ArrayList<Integer>();

		while(listOne.size() > 0 || listTwo.size() > 0){
			if(listOne.size() == 0) {
				newInterwovenList.addAll(listTwo);
				return newInterwovenList;
			}else if(listTwo.size() == 0) {
				newInterwovenList.addAll(listOne);
				return newInterwovenList;
			}
			newInterwovenList.add(listOne.remove(0));
			newInterwovenList.add(listTwo.remove(0));
		}
		return newInterwovenList;
	}
