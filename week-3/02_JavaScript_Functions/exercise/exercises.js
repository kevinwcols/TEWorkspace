...
function isAdmitted(gpa, satScore=0, recommendation=false) {
    if ( gpa > 4.0 || satScore > 1300 ) {
        return true;
    } else if ( ( satScore > 1200 || gpa > 3.0 ) && recommendation === true ) {
        return true;
    } return false;
}
...
function getFullAddressesOfProperties(arrayofObjects) {
    return arrayofObjects.map( (element) => {
        return element.streetNumber + " " + element.streetName + " " + element.streetType + " " +
        element.city + " " + element.state + " " + element.zip });

   /* Alternate way of doing problem is below

        const results = [];
        for (let i = 0; i < arrayofObjects.length; i++) {
            let element = arrayofObjects[i];
            const address = element.streetNumber + " " + element.streetName + " " + element.streetType + " " +
        element.city + " " + element.state + " " + element.zip;
...
function findLargest(searchArray) {
    ...
    searchArray.forEach(function(elem){
        if(largest < elem) 
            largest = elem; });
    return largest;
}
...
function getSumOfSubArrayValues(arr=[]) { 
    return arr.reduce((sum,element) => {
        return (sum + element.reduce( ( minorSum, minorElement)  => { return minorSum + minorElement } ,0) )
    } ,0); 
}