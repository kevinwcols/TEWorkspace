...
public class App {
    ...
    private void loadData() {
        ...
        for (int i=0; i < dataset.length; i++ ) {
            String[] dataRow = dataset[i].split(FIELD_DELIMITER);
            titles.add(dataRow[0]);
            authors.add(dataRow[1]);
            publishedYears.add(Integer.valueOf(dataRow[2]));
            prices.add(new BigDecimal(dataRow[3]));
        }
            ...
            if (mainMenuSelection == 1) {
                ...
                while (true) {
                    ...
                    if (searchBooksMenuSelection == 1) {
                         ...
                         */
                        // displayTitlesList(titles);
                        // optional way:  List<Integer> indexList = filterByTitle(filterTitle);
    ...
    // Challenge 1 - adding primary fields
    private void displaySearchResultsChallenge1(List<Integer> indexes, int primaryField) {

        System.out.println("Results");
        System.out.println("--------");
        // Challenge 2 sort
        SortedSet sorted = new TreeSet<String>();

        System.out.println("");
        for ( Integer index : indexes ) {
            String dataOut = "";
            if ( primaryField == TITLE_FIELD )
                dataOut = (titles.get(index) + " : " + authors.get(index) + " : " + publishedYears.get(index) + " : " + " : $" + prices.get(index));
            else if  ( primaryField == AUTHOR_FIELD ) {
                dataOut = (authors.get(index) + " : " + titles.get(index) + " : " + publishedYears.get(index) + " : " + " : $" + prices.get(index));
            }
            else if ( primaryField == PUBLISHED_YEAR_FIELD ) {
                dataOut = (publishedYears.get(index) + " : " +titles.get(index) + " : " + authors.get(index) + " : " +  " : $" + prices.get(index));
            }
            else if ( primaryField == PRICE_FIELD ) {
                dataOut = (" : $" + prices.get(index) +  " : "  + titles.get(index) + " : " + authors.get(index) + " : " + publishedYears.get(index));
            }
    ...
    private List<Integer> filterByPublishedYearRange(int filterFromYear, int filterToYear) {
        List<Integer> indexListYear = new ArrayList<>();
    ...
    // Returns:-1, 0, or 1 as this BigDecimal is numerically less than, equal to, or greater than val.
        for (int i=0; i < prices.size(); i++ ) {
            if ( prices.get(i).compareTo(BigDecimal.valueOf(filterFromPrice)) >= 0  &&
                    prices.get(i).compareTo(BigDecimal.valueOf(filterToPrice)) <= 0 ) {
                indexList.add((int) i);
            }
        }
        return indexList;
    ...
    private List<Integer> findLeastExpensiveBooks() {
        List<Integer> indexList = new ArrayList<>();
        BigDecimal leastExpensivePrice = BigDecimal.valueOf(20000);
        for (int i=0; i < prices.size(); i++ ) {
            if ( prices.get(i).compareTo(leastExpensivePrice) <= 0 ) {
                leastExpensivePrice = prices.get(i);
            }
        }

        for  (int i=0; i < prices.size(); i++ ) {
            if ( leastExpensivePrice.compareTo(prices.get(i)) == 0  ) {
                indexList.add(i);
            }
        }
