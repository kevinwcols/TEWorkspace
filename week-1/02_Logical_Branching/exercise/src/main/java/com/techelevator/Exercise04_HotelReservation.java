...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        if ( numberOfNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE ) {
            double lessThanThreeNights = numberOfNights * DAILY_RATE;
            return lessThanThreeNights;
        }
        else if ( numberOfNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE ) {
            double moreThanTwoNights = numberOfNights * DISCOUNT_RATE;
            return moreThanTwoNights;
        }
        return 0;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        int numOfWeekdayNights = numOfTotalNights - numOfWeekendNights;

        if ( numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE ) {
            double lessThanThreeNightsNoWeekends = numOfTotalNights * DAILY_RATE;
            return lessThanThreeNightsNoWeekends;
        }
        else if ( numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && numOfWeekendNights < 1) {
            double moreThanThreeNightsNoWeekends = numOfWeekdayNights * DISCOUNT_RATE;
            return moreThanThreeNightsNoWeekends;
        }
        else if ( numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && numOfWeekendNights >= 1) {
            double moreThanThreeNightsWeekends = ( numOfWeekdayNights * DISCOUNT_RATE ) + ( numOfWeekendNights * DAILY_RATE );
            return moreThanThreeNightsWeekends;
        }
        return 0;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights, boolean isRewardsMember) {
        ...
        if ( isRewardsMember == true ) {
            double rewardsMemberDiscNights = numOfTotalNights * DISCOUNT_RATE;
            return rewardsMemberDiscNights;
        }
