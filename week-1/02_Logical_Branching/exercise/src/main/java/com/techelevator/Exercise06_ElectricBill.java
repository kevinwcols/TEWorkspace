...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        if ( unitsUsed <= BASIC_SERVICE_LIMIT ) {
            double lessThanOrEqualTo100UnitsUsedBill = BASIC_SERVICE_RATE * unitsUsed;
            return lessThanOrEqualTo100UnitsUsedBill;
        }
        else if ( unitsUsed > BASIC_SERVICE_LIMIT ) {
            double Over100UnitsUsedBill = ( BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT )  + ( EXCESS_SERVICE_RATE * (unitsUsed - BASIC_SERVICE_LIMIT ) );
            return Over100UnitsUsedBill;
        }
        return 0;
    }
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        if ( unitsUsed <= BASIC_SERVICE_LIMIT && hasRenewableEnergy == false) {
            double lessThanOrEqualTo100UnitsUsedBillNoRenewable = BASIC_SERVICE_RATE * unitsUsed;
            return lessThanOrEqualTo100UnitsUsedBillNoRenewable;
        }
        if ( unitsUsed <= BASIC_SERVICE_LIMIT && hasRenewableEnergy == true) {
            double lessThanOrEqualTo100UnitsUsedBillHasRenewable = (BASIC_SERVICE_RATE * unitsUsed ) - ( (BASIC_SERVICE_RATE * unitsUsed ) * RENEWABLE_ENERGY_DISCOUNT ) ;
            return lessThanOrEqualTo100UnitsUsedBillHasRenewable;
        }
        else if ( unitsUsed > BASIC_SERVICE_LIMIT && hasRenewableEnergy == false ) {
            double Over100UnitsUsedBillNoRenewable = ( BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT )  + ( EXCESS_SERVICE_RATE * (unitsUsed - BASIC_SERVICE_LIMIT ) );
            return Over100UnitsUsedBillNoRenewable;
        }
        else if ( unitsUsed > BASIC_SERVICE_LIMIT && hasRenewableEnergy == true ) {
            double Over100UnitsUsedBillHasRenewableTotal =  ( ( BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT )  + ( EXCESS_SERVICE_RATE * (unitsUsed - BASIC_SERVICE_LIMIT ) ) );
            double Over100UnitsUsedBillHasRenewable = Over100UnitsUsedBillHasRenewableTotal - ( Over100UnitsUsedBillHasRenewableTotal * RENEWABLE_ENERGY_DISCOUNT ) ;
            return Over100UnitsUsedBillHasRenewable;
        }
        return 0;
    }
